...
{
    struct s_proc* p = &proc_tables[pid];
...
/**
  * @brief   <Ring 0> This routine is called after `p_flags' has been set (!= 0),
...
PRIVATE int deadlock(int src, int dest)
{
    struct s_proc* p = proc_tables + dest;
    while (1) {
        if (p->p_flags & SENDING) {
            if (p->p_sendto == src) {
                ...
                do {
                    assert(p->p_msg);
                    p = proc_tables + p->p_sendto;
                    printl("->%s", p->p_name);
                } while (p != proc_tables + src);
                printl("=_=");
...
/**
  * @brief  <Ring 0> Send a message to the dest proc. If dest is blocked waiting
  *         forthe message, copy the message to it and unblock dest. Otherwise 
...
PRIVATE int msg_send(struct s_proc* current, int dest, MESSAGE* m)
{
    ...
    struct s_proc* p_dest = proc_tables + dest; /* proc dest */
...
/**
  * @brief  <Ring 0> Try to get a message from the src proc. If src is blocked sending
...
PRIVATE int msg_receive(struct s_proc* current, int src, MESSAGE* m)
{
    struct s_proc* p_who_wanna_recv = current; /**
                          ...
                          * it.
                          */
    struct s_proc* p_from = 0; /* from which the message will be fetched */
...
PUBLIC void dump_proc(struct s_proc* p)
{
    ...
    sprintf(info, "byte dump of proc_tables[%d]:\n", p - proc_tables); 
