...
/**
  * @brief  <Ring 1> Check hard drive, set IRQ handler,
  *         enable IRQ and initialize data structures.
  */
PRIVATE void init_hd()
{
...
/**
  * @brief <Ring 1> Wait until a disk interrupt occurs.
  */
PRIVATE void interrupt_wait()
{
    MESSAGE msg;
 ...
 * @retval One if sucess, zero if timeout.
 */
PRIVATE int waitfor(int mask, int val, int timeout)
{
    int t = get_ticks();
